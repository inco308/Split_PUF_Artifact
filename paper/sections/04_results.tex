...
experimental campaigns (\texttt{baseline\_mlp\_lr.csv}, lines~2--11;
...
lines~12--16 and 22--26).  At 1{,}500{,}000~CRPs, however, only 3 of 5 runs
...
increases by a factor of approximately 3.4 per XOR level ($N_{50}(6) \approx
3.84\times10^5$ to $N_{50}(7) \approx 1.32\times10^6$).
...
threshold---only 8 of 30 runs succeed (27\%, aggregated across independent campaigns).  These overlapping intervals are consistent with a wide
...
runs succeed (64\%, aggregated across two independent campaigns), with
successful runs achieving $>99\%$ accuracy.
...
at approximately $8.0\times10^6$ CRPs (95\% CI [6.2M,~9.8M]),
...
only 1 of 10 runs succeeds (10\%, aggregated across two protocol variants;
...
Markers are empirical measurements; dashed lines are logistic fits. Error bars denote
...
Taking $N_{50}$ (the data level at which $P(\text{success}) \approx 50\%$) as
a characteristic scale, logistic fits to the full dataset yield the following
estimates: $N_{50}(2) \approx 1.03\times10^5$ ($R^2=0.945$),
$N_{50}(4) < 7.5\times10^4$ (80\% success at lowest tested budget; midpoint unconstrained below),
$N_{50}(6) \approx 3.84\times10^5$ ($R^2=0.863$),
$N_{50}(7) \approx 1.32\times10^6$ ($R^2=0.655$),
$N_{50}(8) \approx 8.0\times10^6$ ($R^2=0.546$, 95\% CI [6.2M,~9.8M]), and
...
However, the 8-XOR fit quality ($R^2 = 0.546$) and preliminary 9-XOR data
caution against treating this as a stable law.  For example,
$N_{50}(8) / N_{50}(7) \approx 6.1$,
$N_{50}(7) / N_{50}(6) \approx 3.4$, and
$N_{50}(9) / N_{50}(8) > 1.9$ (preliminary) show variability across
levels.  Overall, the empirical data complexity grows exponentially in $k$,
...
Table~\ref{tab:lr_mlp} compares MLP and logistic regression (LR) performance
across multiple XOR levels.  The results are unambiguous: logistic regression
produces near-random accuracy ($\approx 50\%$) for all tested configurations
with $k \geq 4$ within the trainable data range.
...
\begin{tabular}{lcccc}
\toprule
$k$ & $N$ & MLP Acc. (mean $\pm$ std) & LR Acc. & LR Success? \\
\midrule
4 & 100k  & $95.45 \pm 6.85\%$ & $50.18 \pm 0.53\%$ & No \\
4 & 200k  & $98.88 \pm 0.19\%$ & $50.26 \pm 0.38\%$ & No \\
6 & 300k  & $64.74 \pm 23.50\%$ & $50.05 \pm 0.22\%$ & No \\
6 & 400k  & $89.40 \pm 20.16\%$ & $50.01 \pm 0.19\%$ & No \\
6 & 500k  & $94.27 \pm 15.56\%$ & $50.12 \pm 0.15\%$ & No \\
...
This result confirms that the XOR operation introduces a nonlinearity that
is fundamentally inaccessible to linear models.  The XOR of $k$ APUF
decisions creates a decision boundary that requires at least one hidden layer
to approximate.  More importantly, it demonstrates that when the MLP succeeds,
it is indeed learning the XOR structure rather than exploiting some linear
artifact in the data.  Polynomial feature expansion (degrees 2 and 3) also
fails on 4-XOR at 100{,}000~CRPs ($\approx 50\%$ accuracy; 3 runs per
degree; results/poly\_lr\_4xor.csv), indicating that the failure of linear
models is not merely a matter of linear feature inadequacy.

% ------------------------------------------------------------
\subsection{Architecture Independence}
\label{sec:arch_independence}

If data quantity is the dominant factor, then model architecture choices
should have negligible impact on attack success below the transition.
We tested this hypothesis on 8-XOR with 2{,}000{,}000~CRPs (well below the
transition midpoint) by varying depth, width, activation functions, residual
connections, and even parameter count by an order of magnitude.
...
The simultaneous variation of multiple architectural dimensions
(depth, width, activation, residuals) in Table~\ref{tab:arch} is a deliberate
\emph{stress test} rather than a single-variable ablation.  While this stress
...
was fine-tuned on 8-XOR with 2{,}000{,}000~CRPs.  The fine-tuned model attains
50.17\% accuracy---identical to training from scratch
(\texttt{8xor\_v2\_results.csv}, line~4, strategy B1).  No knowledge
transfers across XOR complexities, consistent with the view that each XOR
level defines a distinct learning problem with independent information
requirements.
...
logistic sigmoid.  This objective is smooth, and its global optimum is
exactly the true delay vector---so any failure cannot be attributed to
model capacity or a suboptimal parameterization.  PSO used 20 particles
...
CRPs, both well below the fitted midpoint $N_{50}(8) \approx 8\times10^6$.
...
indistinguishable from chance.  This demonstrates that the failure below
the data threshold is not an artifact of MLP optimization: even a smooth
likelihood objective over the correct 520-dimensional delay parameterization
provides no signal at these data volumes.  Architecture independence
therefore extends beyond the MLP family to the level of the optimization
...
APUFs, with the claim that a $(k_{\text{up}}, k_{\text{down}})$-iPUF
provides security equivalent to a $(k_{\text{up}}+k_{\text{down}})$-XOR
APUF~\cite{nguyen2019}.  Wisiol~et~al.~\cite{wisiol2020splitting}
showed that a single configuration, $(3,3)$, can be broken with
$3\times10^6$ CRPs.  We extend this in two directions: a \emph{systematic
sweep} over all configurations up to $(4,4)$, showing that every one falls
at $10^6$ or fewer CRPs, and a \emph{quantitative} characterization of the
resulting effective security level, anchored to our XOR APUF scaling law
(\S\ref{sec:success_curves}).  Our experiments refute the claimed security
level for the tested configurations.

\begin{table}[t]
\centering
\caption{iPUF attack results.  All configurations are attacked successfully
with 1{,}000{,}000~CRPs or fewer.  Equiv.\ XOR refers to the
$k_{\text{up}}+k_{\text{down}}$ sum.  Data source:
...
\midrule
\multirow{3}{*}{$(2,2)$}
  & \multirow{3}{*}[-2pt]{4} & 1{,}000{,}000 & 99.67\% & 809 \\
  &                         & 2{,}000{,}000 & 99.79\% & 809 \\
  &                         & 5{,}000{,}000 & 99.86\% & 809 \\
\midrule
\multirow{3}{*}{$(2,4)$}
  & \multirow{3}{*}[-2pt]{6} & 1{,}000{,}000 & 99.64\% & 6{,}305 \\
...
\midrule
\multirow{3}{*}{$(4,4)$}
  & \multirow{3}{*}[-2pt]{8} & 1{,}000{,}000 & 99.05\% & 20{,}801 \\
...
As shown in Table~\ref{tab:ipuf}, the tested iPUF configurations all
succumb to the split attack with 1{,}000{,}000~CRPs, achieving accuracies
$> 99\%$.
A $(4,4)$-iPUF---claimed to be as secure as an 8-XOR APUF---is attacked with
99.05\% accuracy using 20{,}801 total parameters (Table~\ref{tab:ipuf}).
By contrast, a true 6-XOR APUF
requires 500{,}000~CRPs to achieve comparable reliability (90\% success
probability).  Based on our fitted scaling law, we estimate that a true 8-XOR APUF
would require approximately $8.0\times 10^6$ CRPs---well beyond the
...
XOR APUF does not.  In the tested configurations, the effective security
level is consistent with that of a
$(k_{\text{up}}+k_{\text{down}}-2)$-XOR APUF, i.e., the split attack reduces
the equivalent XOR complexity by exactly 2 for every tested pair
($4\to2$, $6\to4$, $8\to6$).  Because the XOR APUF data complexity grows
super-linearly in $k$, this constant additive reduction translates into a
rapidly growing multiplicative gap in CRP budget: $\approx$5$\times$ at
$(2,4)$ and $\approx$21$\times$ at $(4,4)$ on our fitted scaling law
($N_{50}(6)/N_{50}(4) \approx 5$ and
$N_{50}(8)/N_{50}(6) \approx 21$).  The $(2,2)$ configuration is
uninformative here, as its claimed and effective levels fall in the
anomalous low-$k$ region where $N_{50}$ does not grow monotonically
(\S\ref{sec:success_curves}).  This gap quantification---not the mere
breakability, which is already established---is our principal contribution
on iPUF security.
