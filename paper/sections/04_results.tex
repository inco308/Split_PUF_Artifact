...
$N_{50}(9) / N_{50}(8) > 1.9$ (preliminary) vary across levels.
...
security equivalent to a $(k_{\text{up}}+k_{\text{down}})$-XOR
APUF~\cite{nguyen2019}.  Wisiol~et~al.~\cite{wisiol2020splitting}
broke the single configuration $(3,3)$ with $3\times10^6$ CRPs.  We extend
this in two directions: a \emph{systematic sweep} over all configurations up
to $(4,4)$, showing that every one falls at $10^6$ or fewer CRPs, and a
\emph{quantitative} characterization of the resulting effective security
level, anchored to our XOR APUF scaling law (\S\ref{sec:success_curves}).
...
$8.0\times 10^6$ CRPs for a true 8-XOR APUF---well beyond the
