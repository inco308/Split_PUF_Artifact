...
$n$ switching stages; each stage contains two multiplexers that either pass
the signal straight or cross-switch, depending on the challenge bit.  The
difference in arrival times at two latches determines the response:
\[
...
\]
where $\mathbf{w} \in \mathbb{R}^{n+1}$ is a weight vector representing the
per-stage delay differences.  The weights are distributed as
$\mathbf{w} \sim \mathcal{N}(0, \alpha^2)$ with $\alpha \ll 1$.
...
a manifold whose complexity grows exponentially with $k$.  An adversary who
observes a challenge--response pair $(\mathbf{c}, r_{\text{XOR}})$ learns only
one bit that depends on all $k$ weight vectors simultaneously; the contribution
of any single APUF is masked by the others.
...
APUFs the de facto standard for security-critical PUF applications, despite
growing evidence that practical attack thresholds are lower than the
...
loss~\cite{santikellur2023}.  The hidden layers learn a representation that
approximates the multiplicative XOR interaction.  The number of hidden units
is typically chosen in proportion to $2^{k-1}$ to accommodate the $2^k$
possible XOR configurations.
...
framework~\cite{calypso2024} extended this with particle swarm optimization,
reporting success on 8-XOR with approximately 8M CRPs.

\item \textbf{Deep learning.} Fei~et~al.~\cite{fei2024mope} proposed the
MoPE and MMoPE frameworks that adjust model complexity to PUF complexity,
reporting success on up to 7-XOR.  Their work emphasizes that architectural
matching between model capacity and PUF complexity improves attack efficiency.

\item \textbf{Chosen-challenge attacks.} Sayadi~et~al.~\cite{sayadi2025}
showed that selective choice of challenge patterns can reduce the CRP
requirement by exploiting systematic biases in the PUF response distribution,
achieving success on higher XOR configurations than random-challenge attacks.
\end{itemize}

Despite these advances, prior work has focused on \emph{whether} a given XOR
configuration can be attacked, reporting a single success point or the best
accuracy achieved.  The \emph{probability} of attack success as a function of
CRP budget has not been systematically characterized, which is the gap this
work addresses.
...
$k$-XOR function directly, the split attack trains separate sub-models on
subsets of APUFs and combines their outputs probabilistically.  For a $k$-XOR
...
split-attack-resistant alternative.  An iPUF has a two-layer structure:
\begin{itemize}
\item An upper layer of $k_{\text{up}}$ XORed APUFs produces a response
$r_{\text{up}}$.
\item This response is inserted into the challenge of a lower layer of
$k_{\text{down}}$ XORed APUFs at a fixed bit position.
\end{itemize}
...
this claim by showing that the split attack can be adapted to iPUFs:
the upper layer is attacked independently, and the lower layer is attacked
conditioned on the predicted upper response.  Our results (\S\ref{sec:ipuf})
comprehensively confirm this vulnerability across multiple iPUF configurations
and demonstrate that iPUF security is substantially lower than claimed.
