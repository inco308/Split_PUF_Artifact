...
evolution strategies: Becker~\cite{becker2015cia} applied CMA-ES to 5-XOR and
6-XOR APUFs, achieving above-chance accuracy with substantial CRP budgets, and
the CalyPSO framework~\cite{calypso2024} employed particle swarm optimization
on 8-XOR APUFs, reporting $>95\%$ accuracy with approximately 8M CRPs.  While
...
introduced a chosen-challenge attack strategy for XOR APUFs, demonstrating that
selective challenge sampling reduces the CRP requirement by exploiting
systematic biases in the PUF response distribution.  Wang~et~al.~\cite{wang2024c2d2}
proposed the C2D2 framework that combines contrastive learning with data
augmentation to improve generalization under limited CRP budgets.  While these
...
claimed security equivalence to a 6-XOR APUF.  We extend this result in
...
effective-security characterization anchored to our scaling law---the
split attack reduces the equivalent XOR complexity by exactly 2, yielding a
CRP-budget gap of $\approx$5$\times$ to $\approx$21$\times$---moving beyond
