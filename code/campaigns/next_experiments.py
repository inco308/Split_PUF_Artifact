...
def attack_mlp(xor,tp,tr,ttp,ttr,epochs=500):
    ...
    N=tp.shape[0]; tp=tp.to(device); tr=tr.to(device)
...
CSV='results/next_experiments.csv'
with open(CSV,'w') as f: csv.writer(f).writerow(['name','xor','data','run','mlp_acc','mlp_time','lr_acc','lr_time'])

# 1. 6-XOR 350k/450k过渡点 ×10 (补充S曲线精度)
# 2. 8-XOR 6M (确定100%成功点) ×10
# 3. 4-XOR 100k/200k (低XOR对比) ×10
configs=[
    ('6X_350k',6,350_000,10),
    ('6X_450k',6,450_000,10),
    ('8X_6M',8,6_000_000,10),
    ('4X_100k',4,100_000,10),
    ('4X_200k',4,200_000,10),
]

for name,xor,ds,nr in configs:
    print(f'\n[{name}] {xor}X {ds//1000}k ×{nr}')
    for r in range(1,nr+1):
        ...
        with open(CSV,'a') as f:
            csv.writer(f).writerow([name,xor,ds,r,f'{ma*100:.2f}%',f'{mt:.0f}',f'{la*100:.2f}%',f'{lt:.0f}'])
        gc.collect(); torch.cuda.empty_cache()
print('Done')
